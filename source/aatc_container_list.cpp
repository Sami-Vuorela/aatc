...
namespace aatc {
	namespace container {
		namespace templated {
			...
			void list::sort_funcptr(common::aatc_script_Funcpointer* funcptr, bool ascending) { shared::method::native::sort_funcptr(this,funcptr, ascending); }
		...
		namespace listing {
			...
			template<> void register_container<CONTAINER::LIST>(asIScriptEngine* engine) {
...

				{
					...
					register_containerbase<list>(rs, config::scriptname::container::list);
