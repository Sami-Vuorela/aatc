...
template<> void aatc_register_container<aatc_CONTAINERTYPE::UNORDERED_MAP>(asIScriptEngine* engine, aatc_Initializer* initializer){
	...
	//aatc_container_unordered_map_template::Register_iterator(engine, n_iterator, n_container_T);
	//aatc_container_unordered_map_template::aatc_iterator::Register(engine, n_iterator, n_container_T);
	...
	//r = engine->RegisterObjectMethod(n_container_T, textbuf, asFunctionPtr(aatc_reghelp_construct_hosted_iterator_map_template_end<aect_iterator_shared_map_template<aatc_container_unordered_map_template>, aatc_container_unordered_map_template*>), asCALL_CDECL_OBJLAST); assert(r >= 0);
}
